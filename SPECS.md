# Engineering Specification: Multiplexed PostgreSQL to OpenSearch and Qdrant CDC Pipeline

**Component**: Custom Multi-Topic Rustls Sink Daemon via Native OpenSearch and Qdrant Drivers  
**Telemetry Standard**: OpenTelemetry (OTel) Native Traces, Metrics, and Logs over OTLP  
**Target Environment**: Kubernetes (EKS/GKE)

## 1. Architectural Overview

This system architecture establishes an enterprise-grade, low-latency Change Data Capture (CDC) pipeline shifting transactional mutations from a production PostgreSQL cluster into distinct target sinks: **OpenSearch** (for traditional search indices) and **Qdrant** (for high-performance vector search collections).

Instead of deploying heavy, resource-intensive intermediate message brokers (e.g., Kafka/Kafka Connect), the architecture leverages RisingWave as a cloud-native stream processor and a custom-built, highly concurrent Rust Daemon acting as the secure delivery vehicle. The daemon features a **multi-sink dispatcher** that routes streams based on declarative pipeline configurations.
...
The daemon registers and exposes foundational metrics to your APM scraping engines, tagged explicitly by runtime configuration contexts:

| Metric Name | Type Abstraction | Dimensions / Tags | Functional SLI Target Evaluated |
|---|---|---|---|
| `pipeline_records_ingested` | Counter | `subscription` | Ingestion throughput velocity mapping database extractions |
| `pipeline_records_sunk_success` | Counter | `target_collection` | Effective sink write velocity achieving finality (OS or Qdrant) |
| `pipeline_records_sunk_failed` | Counter | `target_collection` | Failed sink write attempts |
| `pipeline_records_dlq` | Counter | `collection` | Records routed to the local Dead Letter Queue |
| `pipeline_fetch_latency_seconds` | Histogram | `subscription` | Inbound network evaluation execution time profiling |
| `pipeline_flush_latency_seconds` | Histogram | `target_collection` | Outbound sink (OS HTTP bulk / Qdrant gRPC) submission latency profiling |
...
The snippet below mirrors that file and should be kept in sync. It now includes vector embeddings for the Qdrant sink.
...
-- 3. Inbound Product Embeddings Source (For Qdrant Vector Sink)
CREATE TABLE src_postgres_product_embeddings (
    product_id INT,
    name VARCHAR,
    description TEXT,
    embedding FLOAT[1536], -- Pre-computed AI vector embeddings
...
CREATE MATERIALIZED VIEW mv_product_vectors AS 
SELECT product_id, name, description, embedding FROM src_postgres_product_embeddings;

-- 5. High-Performance Changelog Stream Subscriptions
...
CREATE SUBSCRIPTION sub_product_embeddings FROM mv_product_vectors WITH (retention = '1D');
```
...
Build once in CI from a reviewed commit.
Push image with semantic and commit tags.
Deploy to dev using the commit tag or digest.
Promote to staging by reusing the same image digest.
Promote to production by reusing the same image digest.
