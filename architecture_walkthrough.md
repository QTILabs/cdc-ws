...
The system consists of an external **BFF Gateway** (Backend-For-Frontend) implemented in asynchronous Rust (`axum`), which communicates over a high-performance, binary `gRPC` backplane to a private **CDC Daemon** (`tonic`).
...
graph TD
    ...
    subgraph int ["Internal / Private Network (Sovereign Zone)"]
        ...
        Sink[("Target Sink<br/>(OpenSearch)")]
    end
