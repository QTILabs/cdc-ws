...
- **CDC daemon**: reads from the source database subscription, routes data to the configured sinks (OpenSearch and/or Qdrant), and reports metrics over gRPC.
...
- `OS_PASSWORD`: OpenSearch password. **Conditionally required** (only if at least one pipeline uses `sink_type: opensearch`).
...
These settings define where the daemon writes the captured rows. The daemon conditionally initializes sink clients based on the active pipelines to save resources.

#### OpenSearch settings
`OS_URL`: OpenSearch endpoint. Defaults to `https://localhost:9200`.
`OS_USER`: OpenSearch username. Defaults to admin.
`OS_PASSWORD`: OpenSearch password. Required if any pipeline targets OpenSearch.

#### Qdrant settings
`QDRANT_URL`: Qdrant gRPC endpoint. Defaults to `https://localhost:6334`. Must use HTTPS.
...
`PIPELINES_FILE` points to a YAML or TOML file containing pipeline definitions. The daemon uses a unified schema that supports both OpenSearch and Qdrant sinks.
...
- subscription_name: public.customers_sub
  ...
  target_collection: customers_idx
  id_field: id
  batch_size: 1000

# Qdrant Vector Pipeline
...
- `sink_type`: Target sink type. Must be "opensearch" or "qdrant".
- `target_collection`: OpenSearch index name or Qdrant collection name.
- `id_field`: JSON field used as the document/point ID.
- `batch_size`: Number of rows fetched per source read.
- `vector_field`: (Qdrant only) JSON field containing the vector embeddings (e.g., f32 array). Defaults to "embedding" if omitted.
...
Note: the current BFF code only wires GitHub automatically. If you want a second OpenID Connect provider such as `oidc` or `keycloak`, you need to add it in `cdc-bff/src/main.rs` using the same provider pattern.
...
# Sink (Qdrant)
export QDRANT_URL="https://localhost:6334"
...
- Qdrant default gRPC endpoint: `6334`
...
#### Check daemon status
```bash
# Check if daemon is running and get basic health info
cargo run -p cdc-ctl -- status

# Check status with custom daemon URL
cargo run -p cdc-ctl -- status --daemon-url http://localhost:50051

# Output includes:
# - Daemon process status (running/stopped)
# - PID (if running)
...
- The daemon reads the pipelines file at startup. Use the web console "Reload daemon" action or `cdc-ctl reload` after writing a new pipeline configuration.
...
- **Qdrant URLs must use HTTPS**. The daemon enforces TLS for all Qdrant gRPC connections.
- The daemon conditionally initializes sink clients. If no pipelines use Qdrant, the Qdrant gRPC client will not be initialized, saving memory and network resources.
